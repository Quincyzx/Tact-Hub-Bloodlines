...
function RayfieldAdapter:CreateWindow(opts)
    local wrapper = {}

    function wrapper:CreateTab(name, _icon)
        local section = Library:CreateSection(name ~= "" and name or "Tab")
        local tabWrapper = { _section = section }

        function tabWrapper:CreateSection(sectionName)
            -- Aztup lib has one level; reuse the same section, add a label divider if named
            if sectionName and sectionName ~= "" then
                self._section:Label(sectionName)
            end
        ...
        function tabWrapper:CreateToggle(data)
            local cb = data.Callback
            self._section:Toggle(data.Name, function(val)
                if cb then cb(val) end
            end)
        end

        function tabWrapper:CreateSlider(data)
            local props = {
                min = data.Range and data.Range[1] or 0,
                max = data.Range and data.Range[2] or 100,
                default = data.CurrentValue or (data.Range and data.Range[1]) or 0,
                precise = (data.Increment and data.Increment < 1) or false
            }
            local cb = data.Callback
            self._section:Slider(data.Name, props, function(val)
                if cb then cb(val) end
            end)
        end

        function tabWrapper:CreateDropdown(data)
            local cb = data.Callback
            local values = data.Options or {}
            local defaultVal = (data.CurrentOption and data.CurrentOption[1]) or values[1] or ""
            self._section:Dropdown(data.Name, values, function(val)
                if cb then cb({val}) end
            end)
        end

        function tabWrapper:CreateButton(data)
            self._section:Button(data.Name, function()
                if data.Callback then data.Callback() end
            end)
        end

        function tabWrapper:CreateInput(data)
            local cb = data.Callback
            self._section:Box(data.Name, function(val)
                if cb then cb(val) end
            end)
        end

        function tabWrapper:CreateParagraph(data)
            local text = (data.Title or "") .. "\n" .. (data.Content or "")
            local labelHolder = { label = self._section:Label(text) }
            function labelHolder:Set(tbl)
                local newText = (tbl.Title or "") .. "\n" .. (tbl.Content or "")
                if labelHolder.label then
                    labelHolder.label.Text = newText
                end
            end
            return labelHolder
        end

        function tabWrapper:CreateKeybind(data)
            local cb = data.Callback
            self._section:Bind(data.Name or "Keybind", data.CurrentKeybind or Enum.KeyCode.End, function(...)
                if cb then cb(...) end
            end)
            local wrapper = {}
            function wrapper:Set(key) end
            return wrapper
        end
...
function Notify(T, Text, Time, Icon)
    -- Aztup lib does not provide built-in notifications; fall back to SetCore and console
    ...
    print(("[Tact Hub] %s - %s"):format(tostring(T), tostring(Text)))
end
...
-----------------------
Settings:CreateSection("Menu & Configs")
-----------------------

local MenuBind = Settings:CreateKeybind({
   Name = "Menu Toggle",
   CurrentKeybind = "End",
   HoldToInteract = false,
   Flag = "MenuToggleKeybind", 
   Callback = function(Keybind)
